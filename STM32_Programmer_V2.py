...
COMMAND_BL_FLASH_MASS_ERASE			    =0x5E
...
COMMAND_BL_FLASH_MASS_ERASE_LEN                     =6
...
def process_COMMAND_BL_READ_SECTOR_STATUS(length):
    ...
    value1 = read_serial_port(length)
    print("  ",value1)
    s_status = bytearray(value1)
    value = int.from_bytes(s_status, byteorder='little',signed=False)
    print("  ",hex(value))
    #s_status.flash_sector_status = (uint16_t)(status[1] << 8 | status[0] )
    
    print("\n   Sector Status : ", hex(value))
    print("\n  ====================================")
    print("\n  Sector                               \tProtection") 
    print("\n  ====================================")
    if(value & (1 << 15)):
...
def decode_menu_command_code(command):
    ...
    if(command  == 0 ):
       ...
       # print("\n   This command is not supported")
        ...
        data_buf[0] = COMMAND_BL_FLASH_MASS_ERASE_LEN-1 
        data_buf[1] = COMMAND_BL_FLASH_MASS_ERASE
        crc32       = get_crc(data_buf,COMMAND_BL_FLASH_MASS_ERASE_LEN-4) 
        crc32 = crc32 & 0xffffffff
        data_buf[2] = word_to_byte(crc32,1,1)
        data_buf[3] = word_to_byte(crc32,2,1)
        data_buf[4] = word_to_byte(crc32,3,1)
        ...
        for i in data_buf[1:COMMAND_BL_FLASH_MASS_ERASE_LEN]:
            Write_to_serial_port(i,COMMAND_BL_FLASH_MASS_ERASE_LEN-1)
        ...
        sector_num = input("\n   Enter sector number(0-7 or 8-15) Or 255) here :")
        sector_num = int(sector_num, 10)
        if(sector_num != 0xff):
        ...
        for x in range(total_sector):
            ...
            sector_details = sector_numbers[x]
...
def read_bootloader_reply(command_code):
    ...
    if(len(ack) ):
        ...
        if (a_array[0]== 0xA5):
            ...
            elif(command_code) == COMMAND_BL_FLASH_MASS_ERASE:
